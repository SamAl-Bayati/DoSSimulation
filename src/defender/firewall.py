...
class Firewall:
    ...
    def is_blocked(self, ip):
        """Check if an IP is currently in the blocked list.
           If its block time has expired, unblock it immediately."""
    ...
    def check_connection(self, ip):
        """Increment active connections if allowed; block or warn if rate/threshold exceeded."""
        if not self.mitigation_enabled:
            # No mitigations - always allow, increment active
        ...
        if current_rate > self.block_threshold:
            # Block IP for block_time seconds from now
    ...
    def end_connection(self, ip):
        """Called when a connection closes; decrement active count."""
        if self.active_connections > 0:
            self.active_connections -= 1

    def prune_blocked_ips(self):
        """Remove any IP from blocked_ips if its block time has expired,
           without waiting for another connection attempt."""
        current_time = time.time()
